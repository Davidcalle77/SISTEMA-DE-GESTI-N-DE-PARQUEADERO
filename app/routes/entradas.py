...
@entradas_bp.route("/")
def index():
    return render_template("entradas/index.html", historial=EM.obtener_historial(50))
...
def registrar():
    if request.method == "POST":
        ...
        if not all([placa, tipo_vehiculo, tipo_registro]):
            flash(
                "Placa, tipo de vehiculo y tipo de movimiento son obligatorios.",
                "danger",
            )
...
def buscar():
    ...
    return render_template("entradas/buscar.html", resultados=resultados, placa=placa)
